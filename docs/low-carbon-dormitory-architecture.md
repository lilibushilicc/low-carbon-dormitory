...
- 管理端页面直接由各页面或壳组件负责布局。
- 学生端与管理端壳层都对 `router-view` 增加了统一的轻量过渡，减少不同页面间的切换割裂感。
- 根布局还会显示顶部路由进度条，并让导航入口在跳转过程中短暂呈现 pending 反馈。
- 管理端导航和壳层主色已对齐学生端的绿色主题，但仍保留独立的管理员信息结构与菜单分组。
...
6. 新路由默认平滑滚动回到顶部，浏览器后退或前进时保留历史滚动位置。
7. 路由切换状态由 `src/router/route-transition-state.ts` 统一维护，供根布局和导航组件共享。
...
这说明“刷新”并不只是重新查一遍，而是一个带副作用和积分更新的业务动作。  
但当前实现不会再根据系统估算结果自动扣减学生侧水电余额，真实余额只由充值和真实扣费动作改变。
