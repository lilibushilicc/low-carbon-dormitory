...
- `个人积分`：累计到学生账户，用于奖励兑换等个人侧功能
- `宿舍周期积分`：按当前周期宿舍能耗与碳排自动结算，用于排名与看板展示

实现上需要注意：

- 左侧侧栏、奖励中心、水电页中的个人积分应该保持同一口径
- 看板顶部大分值属于宿舍周期积分，不应再被解释成第三种积分
- 如果页面需要补充更多信息，优先展示“积分变化”“排名变化”等衍生指标，不再新增新的积分口径名称
...
- 管理端复用时会额外收口内容最大宽度，避免大屏下总览卡片和表格横向拉伸过度
- 管理端复用时默认每页展示 6 个宿舍，并支持分页切换；卡片区与下方明细表共用同一分页状态，两个区域都会显示当前页、总页数和总数
- 管理端复用时，宿舍明细表会把积分/扣费/总碳合并为“关键指标”，并把状态与更新时间合并展示，以减轻大屏中等宽度下的表格拥挤

### 宿舍状态标签

总览页中的宿舍状态标签由前端根据全局宿舍排名动态分层，不再使用固定的费用/碳排硬阈值：

- 先筛出有有效看板数据的宿舍作为可比集合
- 按接口返回的 `carbonRank` 做分层
- 前 15% 标记为“优秀”
- 15% 到 60% 标记为“良好”
- 60% 到 85% 标记为“预警”
- 末 15% 标记为“异常”
- 对没有有效数据的宿舍，统一降级显示为“预警”

这样可以保证总览页在不同数据规模下都能拉开层次，不会因为固定阈值过死导致大面积显示为同一状态。
...
- `scoreSummary.dormPoints`：个人积分
- `scoreSummary.currentPeriodScore`：宿舍周期积分
- `scoreSummary.scoreDelta`：宿舍周期积分相较上周期的变化值
